...
def main():
    ...
    port_controller = 'COM7'  # 자신의 컨트롤러 시리얼 포트로 수정
    baud_rate = 9600
    ser_sensor = serial.Serial(port_sensor, baud_rate, timeout=1)
    ser_controller = serial.Serial(port_controller, baud_rate, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, bytesize=serial.EIGHTBITS)
    time.sleep(2)  # 시리얼 포트가 안정화될 때까지 잠시 대기

    while True:
        # 데이터 프레임 읽기 (4바이트)
        received_bytes = ser_sensor.read(4)
        if len(received_bytes) == 4 and received_bytes[0] == 0x02:
            ...
            if checksum == (temp_int + humi_int) & 0xFF:
                ...
                # 데이터 전송
                ser_controller.write(DA_CMD_Data_LRC('01', '03', f'{temp_int:02d}{humi_int:02d}0000'))
            else:
